...
public class UserSqlDAO implements UserDAO {
    ...
    public List<User> listUser() {
        ...
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql);
        while(results.next()) {
            User user = mapRowToListUser(results);
            users.add(user);
        }
    ...
    public String returnUsernameById(int userId) {
    	...
    	user = jdbcTemplate.queryForObject("SELECT username From users where user_id = ?", String.class, userId);
    ...
    private User mapRowToListUser(SqlRowSet rs) {
        User user = new User();
        user.setId(rs.getLong("user_id"));
        user.setUsername(rs.getString("username"));
